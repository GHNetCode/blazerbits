...
let dnsResUrl = 'http://localhost:7071/api/dnsRes/' // used for finding Information for ip address`s and Domain names entered..
...
btnArrHvr.addEventListener("pointerdown", async function(e) {
  ...
  if (srchInpTxt.value === "") {
    ...
    url = 'http://localhost:7071/api/ipgeoApi/';
  ...
  //We have something in the search field, validate string..
  ...
  if (chkDomIpvalid.length > 0 && chkDomIpvalid.length < 4) {
         ...
         if (dnsdata) {
           ...
           if (Array.isArray(dnsdata) && dnsdata.length > 0) {
             ...
             if (dnsdata[0]) {
               ...
               inpTxtHasIp = true;
               url = 'http://localhost:7071/api/ipgeoApi/' + dnsdata[0];
     ...
     //  return;
       if (inpTxtHasDom==false){ // Domain invalid or not found, next step is check if it`s a valid IPV6 address(v6Regex)..
          //--console.log('inpTxtHasDom :'+inpTxtHasDom)

          const v6Regex = /^(([0-9a-fA-F]{1,4}:){7,7}[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,7}:|([0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,5}(:[0-9a-fA-F]{1,4}){1,2}|([0-9a-fA-F]{1,4}:){1,4}(:[0-9a-fA-F]{1,4}){1,3}|([0-9a-fA-F]{1,4}:){1,3}(:[0-9a-fA-F]{1,4}){1,4}|([0-9a-fA-F]{1,4}:){1,2}(:[0-9a-fA-F]{1,4}){1,5}|[0-9a-fA-F]{1,4}:((:[0-9a-fA-F]{1,4}){1,6})|:((:[0-9a-fA-F]{1,4}){1,7}|:)|::(ffff(:0{1,4}){0,1}:){0,1}((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])|([0-9a-fA-F]{1,4}:){1,4}:((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9]))$/;
            function isValidIPv6Regex(srchInpTxtcleaned) {
                          return v6Regex.test(srchInpTxtcleaned);
             }

             if (isValidIPv6Regex(srchInpTxtcleaned)){
              //--console.log('isValidIPv6Regex:'+isValidIPv6Regex(srchInpTxtcleaned))

                //--console.log("[80a] btnArrHvr - ✅ Valid IP address:", srchInpTxtcleaned);
                inpTxtHasIp = true;
                url = 'http://localhost:7071/api/ipgeoApi/' + srchInpTxtcleaned;
              ...
              //--console.log('isValidIPv6Regex:  ❌'+isValidIPv6Regex(srchInpTxtcleaned))
                alert('Invalid domain name or IPV6 entered: ' + srchInpTxtcleaned);
                btnArrHvrStyle();
                rotateArrow.cancel();
                //--console.log("[80] btnArrHvr - Exiting due to invalid domain or IPV6.");
                return;
  ...
  } else if (chkDomIpvalid.length === 4) { //Domain check above failed, Check if it's a valid IP address
...
async function getmap(lti, lgi, width, height) {
    ...
    try {
        const osmUrl = `http://localhost:7071/api/osmApi/${lti}/${lgi}`;
